...
import com.orange.carshow.services.dto.Details;
...
public class CarServiceImpl implements CarService {
    ...
    private List<CardDto> carLits = new ArrayList<>();
    private final ObjectMapper mapper;

    public CarServiceImpl(ObjectMapper mapper) {
        this.mapper = mapper;
    }

    @PostConstruct
    public void initList() throws Throwable, JsonProcessingException {
    ...
    @Override
    public CardDto get_most_expensivecar() {
        CardDto most_expensive = null;
        for (CardDto c : carLits) {
            if (most_expensive == null || c.getPrice() > most_expensive.getPrice()) {
                most_expensive = c;
            }
        }
    ...
    @Override
    public CardDto get_car_by_id(long modelID) {
        for (CardDto id : carLits) {
            if (id.getModelID() == modelID) {
                return id;
            }
        }
        throw new RuntimeException("Car not found with modelID: " + modelID);
    }

//    public CardDto num_of_car() {
//        int num;
//        Scanner scanner = new Scanner(System.in);
//        System.out.println("enter the neumber of car you want to add it : ");
//        num = scanner.nextInt();
//        for (int i = 0; i < num; i++) {
//            CardDto newcar = new CardDto();
//           carLits.add(newcar);
//        }
//
//    }
    ...
    @Override
    public CardDto addcar(CardDto cardDto) {
        CardDto newcar = new CardDto();
        newcar.setThumbnail(cardDto.getThumbnail());
        newcar.setMakeID(cardDto.getModelID());
        newcar.setModel(cardDto.getModel());
        newcar.setMakerDetails(cardDto.getMakerDetails());
        newcar.setModel(cardDto.getModel());
        newcar.setPrice(cardDto.getPrice());
        newcar.setYear(newcar.getYear());
        carLits.add(newcar);
        return cardDto;
    }

    @Override
    public CardDto delete_byid(long modelID) {
        Iterator<CardDto> iterator = carLits.iterator();
        while (iterator.hasNext()) {
            CardDto car = iterator.next();
    ...
    @Override
    public List<CardDto> search_car(String model) {
        if (model == null || model.isBlank()) {
            return carLits;
        }
        List<CardDto> search = new ArrayList<>();
        for (CardDto c : carLits) {
            if (c.getModel().getNameEn().equalsIgnoreCase(model)
                    || c.getModel().getNameAr().equalsIgnoreCase(model)
                    || c.getMakerDetails().getNameEn().equalsIgnoreCase(model)
                    || c.getMakerDetails().getNameAr().equalsIgnoreCase(model)) {
